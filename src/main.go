...
func main() {
	...
	adminServer := web.NewSdkHttpServer("admin-test-server",
		// 注意，如果你真实环境里面，使用的是多个server监听不同端口
	...
	// 准备静态路由
	staticHandler := web.NewStaticResourceHandler("demo/static", "/static",
	...
	web.WaitForShutdown(
		...
		web.BuildCloseServerHook(server),
		func(ctx context.Context) error {
	...
	// filterName := ReadFromConfig
	// 匿名引入之后，就可以在这里按名索引 filter
	// web.NewSdkHttpServerWithFilterName("my-server", filterNames...)
}
