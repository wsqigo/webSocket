...
func WaitForShutdown(hooks ...Hook) {
	...
	case sig := <-signals:
		...
		for _, hook := range hooks {
			...
			err := hook(ctx)
			if err != nil {
				fmt.Printf("failed to run rook, err: %v \n", err)
			}
			cancel()
		}
		os.Exit(0)
...
// 只需要声明 bool，然后在关闭的时候设置为 true。在这里直接检测 true or false 就可以了。
// 这样做法有一个很重要的点，在设置值的时候，即使 bool 被高速缓存缓存了，
// 即使 bool 在平台上，处理器并不能一条指令设置好值，
// 但是也没什么关系。因为我们可以确认，最终 bool 会变成 true
// 这个做法更加难以理解，所以采用了 closing int32 的做法
