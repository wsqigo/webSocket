...
func (h *StaticResourceHandler) ServeStaticResource(ctx *Context) {
	req := strings.TrimPrefix(ctx.R.URL.Path, h.pathPrefix)
	if item, ok := h.readFileFromData(req); ok {
		fmt.Printf("read data from cache...")
		h.writeItemAsResponse(item, ctx.W)
		return
	}
...
func (h *StaticResourceHandler) writeItemAsResponse(item *fileCacheItem, writer http.ResponseWriter) {
	...
	writer.Header().Set("Content-Type", fmt.Sprintf("%d", item.fileSize))
